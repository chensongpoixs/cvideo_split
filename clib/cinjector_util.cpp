...
#define UPPER_HALFBYTE(x) (((x) >> 4) & 0xF)
	...
	namespace injector_util
	{
		bool   injector_dll(DWORD process_id, bool injector, bool p2p    )
		{
#ifdef _MSC_VER
			/*if (injector)
			{
			...
			WCHAR rtc_load_path[MAX_PATH] = L"";
			WCHAR cloud_rendering_load_path[MAX_PATH] = L"";
			WCHAR capture_hook64_load_path[MAX_PATH] = L"";
			WCHAR NvStreamrtc_load_path[MAX_PATH] = L"";
			GetCurrentDirectoryW(MAX_PATH, rtc_load_path);
			GetCurrentDirectoryW(MAX_PATH, cloud_rendering_load_path);
			GetCurrentDirectoryW(MAX_PATH, capture_hook64_load_path);
			GetCurrentDirectoryW(MAX_PATH, NvStreamrtc_load_path);
			wcscat_s(rtc_load_path, L"\\RenderServer\\runtime\\");
			wcscat_s(cloud_rendering_load_path, L"\\RenderServer\\runtime\\");
			wcscat_s(capture_hook64_load_path, L"\\RenderServer\\runtime\\");
			wcscat_s(NvStreamrtc_load_path, L"\\RenderServer\\runtime\\");
			wcscat_s(rtc_load_path, L"rtc.dll");
			wcscat_s(cloud_rendering_load_path, L"cloud_rendering.dll");
			wcscat_s(capture_hook64_load_path, L"capture_hook64.dll");
			wcscat_s(NvStreamrtc_load_path, L"NvStreamRtc.dll");
			if (!p2p)
			{
				...
				{
					cinjector_util::inject_library_full(process_id, rtc_load_path);
				}

				if (GetFileAttributesW(cloud_rendering_load_path) == INVALID_FILE_ATTRIBUTES)
				{
					WARNING_EX_LOG("Failed to find RenderServer\\runtime\\cloud_rendering.dll !!!");
					return false;
				}
				else if (injector)
				{
					cinjector_util::inject_library_full(process_id, cloud_rendering_load_path);
				}

				if (GetFileAttributesW(capture_hook64_load_path) == INVALID_FILE_ATTRIBUTES)
				{
					WARNING_EX_LOG("Failed to find RenderServer\\runtime\\capture_hook64.dll !!!");
					return false;
				}
				else if (injector)
				{
					cinjector_util::inject_library_full(process_id, capture_hook64_load_path);
				}
				// remove_thread 
				NORMAL_EX_LOG("rtc.dll , cloud_rendering.dll, capture_hook64.dll OK  !!!");
			}
			else
			{
				if (GetFileAttributesW(NvStreamrtc_load_path) == INVALID_FILE_ATTRIBUTES)
				{
					WARNING_EX_LOG("Failed to find RenderServer\\runtime\\NvStream.dll !!!");
					return false;
				}
				else if (injector)
				{
					cinjector_util::inject_library_full(process_id, NvStreamrtc_load_path);
				}
				// remove_thread 
				NORMAL_EX_LOG("NvStreamRtc.dll OK  !!!");
			}
#endif
