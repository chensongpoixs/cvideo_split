...
namespace chen {
	...
	private:
		...
		unsigned int					m_max_block_size; // ×î´óblockµÄ¸öÊýÊ± gc
		unsigned int					m_block_size; // Ã¿¸öblockµÄ´óÐ¡
		unsigned int					m_start_size; // ÆðÊ¼Î»ÖÃ
		unsigned int					m_pool_size; //³Ø×ÓµÄ¸öÊý
		unsigned int					m_current_size; //¿ÉÒÔÊ¹ÓÃµÄÄÚ´æ´óÐ¡
		unsigned int					m_block_count; //ÉêÇëÁËÄÚ´æµÄ¿éÊý
		unsigned int					m_use_size; //ÒÑ¾­ÁËÄÚ´æµÄ´óÐ¡
		unsigned int					m_free_size; //ÏÐÖÃÄÚ´æ¿éµÄÊýÁ¿
		clock_type						m_lock;
		bool							m_safe;
	};
} // namespace chen
