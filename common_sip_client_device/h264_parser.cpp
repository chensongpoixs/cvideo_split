...
namespace chen {
	...
	int GetAnnexbNALU(NALU_t* nalu) {
		...
		if ((Buf = (unsigned char*)calloc(nalu->max_size, sizeof(char))) == NULL)
			NORMAL_EX_LOG("GetAnnexbNALU: Could not allocate Buf memory\n");
		...
		if (0 != fseek(h264bitstream, rewind, SEEK_CUR)) {
			free(Buf);
			NORMAL_EX_LOG("GetAnnexbNALU:Cannot fseek in the bit stream file");
		}
		...
		//int simplest_h264_parser(const char *url)
	{
		...
		if (n == NULL) {
			WARNING_EX_LOG("Alloc NALU Error");
			exit(1);
		}
		...
		if (n->buf == NULL) {
			free(n);
			WARNING_EX_LOG("AllocNALU:n->buf");
			exit(1);
		}

		int data_offset = 0;
		int nal_num = 0;
		NORMAL_EX_LOG("-----+-------- NALU Table ------+---------+\n");
		NORMAL_EX_LOG(" NUM |    POS  |    IDC |  TYPE |   LEN   |\n");
		NORMAL_EX_LOG("-----+---------+--------+-------+---------+\n");
