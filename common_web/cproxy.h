...
namespace chen
{
	...
	public:
		...
		template <typename C,
			...
			typename T6,
			typename T7>
			void Invoke(C* c, M m, T1 a1, T2 a2, T3 a3, T4 a4, T5 a5, T6 a6, T7 a7) {
			r_ = (c->*m)(std::move(a1), std::move(a2), std::move(a3), std::move(a4),
				std::move(a5), std::move(a6), std::move(a7));
		}
	...
	template <typename C,
		...
		typename T5,
		typename T6>
		class MethodCall6 : public MessageHandler {
		public:
		...
		private:
			void OnMessage(Message*) {
				r_.Invoke(c_, m_, std::move(a1_), std::move(a2_), std::move(a3_),
					std::move(a4_), std::move(a5_), std::move(a6_));
			}
	...
	template <typename C,
		...
		typename T6,
		typename T7>
		class MethodCall7 : public MessageHandler {
		public:
			typedef R(C::* Method)(T1 a1, T2 a2, T3 a3, T4 a4, T5 a5, T6 a6, T7 a7);
			MethodCall7(C* c, Method m, T1 a1, T2 a2, T3 a3, T4 a4, T5 a5, T6 a6, T7 a7)
				: c_(c),
				m_(m),
				a1_(std::move(a1)),
				a2_(std::move(a2)),
		...
		private:
			void OnMessage(Message*) {
				r_.Invoke(c_, m_, std::move(a1_), std::move(a2_), std::move(a3_),
					std::move(a4_), std::move(a5_), std::move(a6_), std::move(a7_));
			}
...
#define PROXY_WORKER_METHOD3(r, method, t1, t2, t3 )										\
  r method(t1 a1, t2 a2, t3 a3 )  {														\
    MethodCall3<C, r, t1, t2, t3> call(&g_web_http_api_mgr , &C::method,  std::move(a1), std::move(a2), std::move(a3));					\
	r r1 = call.Marshal(CHEN_FROM_HERE, &g_http_queue_mgr);												\
	return 	r1;																							\
  }
...
#define PROXY_WORKER_METHOD4(r, method, t1, t2, t3, t4)												\
  r method(t1 a1, t2 a2, t3 a3, t4 a4 )  {														\
    MethodCall4<C, r, t1, t2, t3, t4> call(&g_web_http_api_mgr , &C::method,								\
...
#define PROXY_WORKER_METHOD5(r, method, t1, t2, t3, t4, t5)												\
  r method(t1 a1, t2 a2, t3 a3, t4 a4, t5 a5)  { 																	\
    MethodCall5<C, r, t1, t2, t3, t4, t5> call(&g_web_http_api_mgr , &C::method,								\
                                      std::move(a1), std::move(a2), std::move(a3), std::move(a4), std::move(a5));		\
	r r1 = call.Marshal(CHEN_FROM_HERE, &g_http_queue_mgr);												\
	return 	r1;																							\
  }

#define PROXY_WORKER_METHOD6(r, method, t1, t2, t3, t4, t5, t6)												\
  r method(t1 a1, t2 a2, t3 a3, t4 a4, t5 a5, t6 a6)  { 																	\
    MethodCall6<C, r, t1, t2, t3, t4, t5, t6> call(&g_web_http_api_mgr , &C::method,								\
                                      std::move(a1), std::move(a2), std::move(a3), std::move(a4), std::move(a5), std::move(a6));		\
	r r1 = call.Marshal(CHEN_FROM_HERE, &g_http_queue_mgr);												\
	return 	r1;																							\
  }

#define PROXY_WORKER_METHOD7(r, method, t1, t2, t3, t4, t5, t6, t7)												\
  r method(t1 a1, t2 a2, t3 a3, t4 a4, t5 a5, t6 a6, t7 a7)  { 																	\
    MethodCall7<C, r, t1, t2, t3, t4, t5, t6, t7> call(&g_web_http_api_mgr , &C::method,								\
                                      std::move(a1), std::move(a2), std::move(a3), std::move(a4), std::move(a5), std::move(a6), std::move(a7));		\
	r r1 = call.Marshal(CHEN_FROM_HERE, &g_http_queue_mgr);												\
	return 	r1;																							\
  }
