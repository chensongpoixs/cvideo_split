    ...
    class Response : public std::enable_shared_from_this<Response>, public std::ostream {
      ...
      void write_header( const CaseInsensitiveMultimap &header, size_type size) {
        ...
        bool event_stream = false;
         SimpleWeb::CaseInsensitiveMultimap header1;
         header1.emplace("Access-Control-Allow-Methods", "GET, POST, OPTIONS, PUT, DELETE");
         header1.emplace("Access-Control-Request-Headers", "Content-Type");
         header1.emplace("Access-Control-Allow-Headers", "DNT,User-Agent,X-Requested-With,If-Modified-Since,Cache-Control,Content-Type,Range,x-api-key,x-token");
         header1.emplace("Access-Control-Allow-Origin", "*");
       // response->write(header);
         for (auto& field : header1) {
    ...
    void read(const std::shared_ptr<Session> &session) {
      ...
      asio::async_read_until(*session->connection->socket, session->request->streambuf, "\r\n\r\n", [this, session](const error_code &ec, std::size_t bytes_transferred) {
        ...
        if(!ec) {
         ...
         // std::string str(std::istreambuf_iterator<char>(session->request->content ), eos);
         // NORMAL_EX_LOG("[method = %s][content = %s]", session->request->method, str.c_str());
               ...
               std::function<void(std::shared_ptr<typename ServerBase<socket_type>::Response>, std::shared_ptr<typename ServerBase<socket_type>::Request>)> &resource_function) 
    {
      ...
      try {
         /* SimpleWeb::CaseInsensitiveMultimap header;
          ...
          header.emplace("Access-Control-Request-Headers","Content-Type");
          header.emplace("Access-Control-Allow-Headers","DNT,User-Agent,X-Requested-With,If-Modified-Since,Cache-Control,Content-Type,Range,x-api-key,x-token");
