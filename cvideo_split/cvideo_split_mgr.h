...
namespace chen {
	...
	public:
		void   push_stop_video(const std::string & channel_name);
	private:
	...
	private:
		...
		std::mutex				m_video_split_lock;
		std::vector<std::string> m_video_split_list;

		std::map<std::string, uint64>  m_video_split_restart;
