...
namespace chen {
	...
	{
		_register_msg_handler(C2S_PlayUrl, "C2S_PlayUrl", &cwebsocket_wan_session::handler_play_url);
		return true;
	}
	...
	cclient_msg_handler * cclient_msg_dispatch::get_msg_handler(uint16 msg_id)
	{
		...
		if (/*(msg_id < S2C_WebrtcMessageUpdate) || */(msg_id < Msg_Client_Max  || msg_id > C2S_BeatHeart))
		{
