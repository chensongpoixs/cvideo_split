...
#define PARSE_NAME_VALUE(VAR, NAME, TYPE) 							\
	...
	bool cweb_http_api_mgr::init()
	{
		...
		REGISTER_WEB_HANDLER("update_render_app", "POST", std::bind(&cweb_http_api_mgr::_handler_update_render_app, this, std::placeholders::_1, std::placeholders::_2));
		REGISTER_WEB_HANDLER("delete_render_app", "POST", std::bind(&cweb_http_api_mgr::_handler_delete_render_app, this, std::placeholders::_1, std::placeholders::_2));

		REGISTER_WEB_HANDLER("cmd_render_app", "POST", std::bind(&cweb_http_api_mgr::_handler_cmd_render_app, this, std::placeholders::_1, std::placeholders::_2));
	...
	cresult_app_info cweb_http_api_mgr::update_render_app(const update_render_app_struct& msg)
	{
		cresult_app_info result;
		return result; //g_global_logic_mgr.handler_web_update_render_app(msg);;;
	}

	uint32_t cweb_http_api_mgr::delete_render_app(const std::string& app_id)
	{
		cresult_app_info result;
		return EWebSuccess; // g_global_logic_mgr.handler_web_delete_render_app(app_id);;;
	}

	uint32_t cweb_http_api_mgr::cmd_render_app(const cmd_render_app_struct& msg)
	{
		//return g_global_logic_mgr.handler_web_cmd_render_app(msg);;;
		return EWebSuccess;
		//if (0 != cmd)
		//{
		////	return g_global_render_app_mgr.central_stop_render_app(app_render);
		//}
		//return g_global_render_app_mgr.central_start_render_app(app_render);
	}
}