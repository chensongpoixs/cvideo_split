    ...
    function LoadVideo(data, num) {
        ...
        if (num > 0 && typeof(data)!='undefined'&&data!=null&&data!='') {
            for (var i = 1; i <= num; i++) {
                ...
                //var  cam = _.find(tileArray, function (obj) { return i == obj.idx_in_tile });//获取此通道第几个下拉列表需要选中的值
                //循环绑定所有通道未使用的相机
...
function LoadTileVideo()
{
    $.each(tileArray,function(index,item){
           var cam = _.find(tileArray, function (obj) { return index == obj.idx_in_tile });
            $("#videoName"+(index+1)).val(cam.camera_id);
    })
    layui.use(['form'], function () {
        var form = layui.form;
        form.render();
