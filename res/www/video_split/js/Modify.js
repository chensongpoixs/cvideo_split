...
function InitCoordinate(num, t, l, d, r) {
    ...
    console.log('InitCoordinate num= ' + num + 'x =' + t + ', y = ' + l + "w = " + d + ", h = " + r, bgwidth, bgheight);
    // $("#up"+num).val(parseInt(bgheight));
    // $("#down"+num).val(parseInt(bgheight));
    // $("#left"+num).val(parseInt(bgheight));
    // $("#right"+num).val(parseInt(bgwidth));


    // $("#width" + num).val(parseInt(d * bgwidth));
    // $("#height" + num).val(parseInt(r * bgheight));
    // $("#top" + num).val(parseInt(l * bgheight));
    // $("#left" + num).val(parseInt(t * bgwidth));
    // TODO@chensong 2024-06-12 框子
    // 初始事件


    setTimeout(() => {
        $("#zxxCropBox" + num).css("width", d * bgwidth);
        $("#zxxCropBox" + num).css("height", r * bgheight);
        $("#zxxCropBox" + num).css("top", l * bgheight);
        $("#zxxCropBox" + num).css("left", t * bgwidth);
        console.log('执行1', d * bgwidth, r * bgheight, l * bgheight, t * bgwidth)
    }, 100);
    ...
    for (let i = 0; i < tileArray.length; ++i) {
        ...
        for (let j = 0; j < data.length; ++j) {
            ...
            if (data[j].camera_id === tileArray[i].camera_id) {
                ...
                // console.log(' i = ' + i + ', c');
                $("#videoName" + (parseInt(tileArray[i].index) + 1)).val(data[j].camera_id);
...
function ChangeVideoNmae(camera_id, img, num) {
    ...
    if (tileArray.length > 0 && num - 1 < tileArray.length)//初始化的数量存在和手动选择拼接方式数值不一样
    {
        for (let index = 0; index < tileArray.length; index++) {
            if (tileArray[index].index == (num - 1)) {
                console.log('初始变量框子', num, tileArray[index].x, tileArray[index].y, tileArray[index].w, tileArray[index].h)
                InitCoordinate(num, tileArray[index].x, tileArray[index].y, tileArray[index].w, tileArray[index].h);
            }
        }
    }
...
let videoMsg = function (num) {
    ...
    if (num > 0) {
        for (var i = 1; i <= num; i++) {
            ...
            //转换0-1坐标
            up = parseInt($("#zxxCropBox" + i).css("top")) / bgheight;
            down = parseInt(down) / bgheight;
            left = parseInt($("#zxxCropBox" + i).css("left")) / bgwidth;
            right = parseInt(right) / bgwidth;
            cropImageWidth = parseInt($("#zxxCropBox" + i).css("width")) / bgwidth;
            cropImageHeight = parseInt($("#zxxCropBox" + i).css("height")) / bgheight;
            //tileArray[]


            camera_infos_group.push({
                camera_id: parseInt(videoName),
                index: parseInt((i - 1)),
                x: left,
                y: up,
                w: cropImageWidth,
