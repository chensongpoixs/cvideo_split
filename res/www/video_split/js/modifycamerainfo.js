...
function GetParams()//如果是修改拼接，需要解析URL传递过来的参数
   {
        ...
        if(vars.length===5)
        {
            camera_id = parseInt(vars[0].split('=')[1]);
            $("#cameraName").val(decodeURIComponent(vars[1]).split('=')[1]);
            $("#cameraAddress").val(vars[2].split('=')[1]);
            $("#multicastAddress").val(vars[3].split('=')[1]);
            $("#multicastPort").val(vars[4].split('=')[1]);
  ...
  //获取摄像头信息
    ...
    function modify_camera_info()
    {
    	console.log('modify_camera_info');
    	  let cameraName = $("#cameraName").val();
    	  let cameraAddress  = $("#cameraAddress").val();
    	  let multicastAddress  = $("#multicastAddress").val();
    	  let multicastPort  = $("#multicastPort").val();
    	  ...
    	  SendAddOneCameraInfo({  'camera_id': camera_id, 'ip': cameraAddress, 'camera_name': cameraName, 'port': multicastPort, 'address': multicastAddress});
    }

    function modify_camera_player_play()
    {

    	console.log('modify_camera_player_play');
    	  let cameraName = $("#cameraName").val();
    	  let cameraAddress  = $("#cameraAddress").val();
    	  let multicastAddress  = $("#multicastAddress").val();
    	  let multicastPort  = $("#multicastPort").val();
    ...
    //发送execl导入请求
        function SendAddOneCameraInfo(data) 
        {
            ...
            $.ajax({
                type: "POST",
                dataType: "text",
                url: video_split_address + '/video_split/v1/modify_camera_info',
